package ma.zs.budgetInstitut.service.facade.admin.achat;

import java.math.BigDecimal;
import java.util.List;
import ma.zs.budgetInstitut.bean.core.achat.TypeAchatMateriel;
import ma.zs.budgetInstitut.dao.criteria.core.achat.TypeAchatMaterielCriteria;
import ma.zs.budgetInstitut.zynerator.service.IService;



public interface TypeAchatMaterielAdminService extends  IService<TypeAchatMateriel,TypeAchatMaterielCriteria>  {


    // New method for determining purchase type based on total amount



}
